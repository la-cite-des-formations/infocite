<div wire:ignore.self id="icon-picker" @class([
    'col-auto dropend',
    'admin-interface' => isset($inAdminInterface),
])>
    ...
    <a href="https://fonts.google.com/icons?icon.set=Material+Icons" type="button" class="ms-1" target="_blank"
    ...
    <div wire:ignore.self class="dropdown-menu text-muted bg-icon-picker ms-1 p-1 border-0" style="min-width: 410px;">
        <div class="input-group mb-1 border-0">
            <div class="input-group-text text-secondary border-0">
                <span class="material-icons">search</span>
            </div>
            <input wire:model='searchIcons' type="text" class="form-control search-icons border-0"
                placeholder="Rechercher...">
        </div>
        <div id="{{ $model }}-icon" wire:model="{{ $model }}.icon" type="input"
            style="max-width: 410px; max-height: 245px;"
            ...
            @if (AP::getRecentMiCodes()->isNotEmpty())
                @foreach (AP::getRecentMiCodes() as $recentMi)
                    <button wire:click="choiceIcon('{{ $recentMi['name'] }}', '{{ $model }}')" type="button"
                        value='{{ $recentMi['name'] }}' title='{{ $recentMi['name'] }}'
            ...
            @foreach ($icons as $miName => $miCode)
                <button wire:click="choiceIcon('{{ $miName }}', '{{ $model }}')" type="button"
                    value='{{ $miName }}' title='{{ $miName }}'
