        ...
        <div class="container d-flex flex-column">
            <div class="align-self-end">
                <div class="input-group" role="group">
                    <a href="{{ route('personal-apps.create') }}" title="Ajouter une application personnelle" type="button"
                        ...
                        <tr wire:click="redirectToApp('{{ $app->url }}')" role="button">
                            <td class="p-0">
                                <h3 class="d-flex p-3 m-0">
                                    ...
                                    @else
                                        <img src="{{ $app->favicon }}" class="me-2"
                            ...
                            <td class="pe-3">
                                <div wire:click.prefetch='blockRedirection' class="input-group justify-content-end"
                                    role="group" aria-label="Actions">
                                    @can('handle', $app)
                                        <a href="{{ route('personal-apps.edit', ['app_id' => $app->id]) }}" role="button"
                                            class="btn btn-sm btn-success" title="Modifier">
                                            <i class="bx bx-pencil"></i>
                                        </a>
                                        <button
                                            wire:click="$emitTo('modal-manager', 'show', {component: 'usage.confirm', data: {handling : 'deleteApp', appId : {{ $app->id }}}})"
                                ...
                                <a>
                                    <h3 class="d-flex">
                                        @if (empty($app->favicon))
                                            <span class="material-icons ms-0 me-2">{{ $app->icon }}</span>
                                        @else
                                            <img src="{{ $app->favicon }}" class="logo me-2"
                                                style="width: 25px; height: 25px;">
                                        @endif
                                        {{ $app->name }}
                                    </h3>
